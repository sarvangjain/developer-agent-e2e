# ✋ CHECKPOINT 2: Architecture & Design Document (Draft 4)
...
**Revision:** Draft 4 — incorporates feedback from Drafts 1, 2, and 3
...
| # | Section | Change | Reason |
|---|---------|--------|--------|
| 8 | Instance Task State Machine | **Restored** document-deletion reversion flow (COMPLETED → PENDING on doc delete) to the backend. Added `onDocumentDeleted()` callback via direct integration with document service (Option A). | Feedback D3: backend must handle task status reversion when a document is deleted — not a UI concern |
| 9 | Request Flow | Added `onDocumentDeleted` internal callback flow under Execution Phase | Feedback D3: document the reversion request path |
| 10 | Files to Modify | Added `document_service/index.js` to the modification list — callback to `checklistService.onDocumentDeleted()` after successful deletion | Feedback D3: document service needs a new integration point |
| 11 | Database Table Design | Restored `REVERTED_DOC_DELETED` to the audit action list | Feedback D3: audit trail must record document-deletion reversions |
| 12 | Integration Points | Added **Document Service (Deletion Callback)** as a dedicated integration point | Feedback D3: make the deletion integration explicit |
...
#### Internal Callback: `onDocumentDeleted` — Revert Task on Document Deletion
```
Document Service (document_service/index.js#deleteDocument)
  → After successful deletion
  → Service (checklist/index.js#onDocumentDeleted(trx, workflowId, documentType))
    → DB Service (checklist.js#getInstanceTasksByDocDeletion) — find tasks where:
        workflow_id matches AND completion_criteria = 'DOCUMENT'
        AND document_type matches AND status = 'COMPLETED'
    → For each matching task:
        → DB Service (checklist.js#updateInstanceTask) — status: COMPLETED → PENDING, clear completed_by/completed_at
        → DB Service (checklist.js#insertTaskAudit) — action: REVERTED_DOC_DELETED
    → If any reverted task is is_mandatory = true:
        → Notification Service — N3.1 (mandatory document removed alert)
```

**Note:** This is an internal service-to-service callback, not an HTTP endpoint. The document service calls `checklistService.onDocumentDeleted()` directly after a successful document deletion within the same transaction.
...
| `backend/src/lending/services/checklist/index.js` | Service | Business logic for template CRUD, instance lifecycle, workflow actions, `onDocumentDeleted()` callback | `backend/src/lending/services/credit_note/index.js` |
...
| `backend/src/lending/services/document_service/index.js` | Document deletion function(s) | Add callback to `checklistService.onDocumentDeleted(trx, workflowId, documentType)` after successful document deletion | Document deletion flow — must call checklist reversion within same transaction |
...
**Task Reversion on Document Deletion (COMPLETED → PENDING):** When a Document-type task is in `COMPLETED` status and the corresponding document (matched by `workflow_id` + `document_type`) is deleted via the document service, the backend reverts the task status from `COMPLETED` to `PENDING`. This reversion is handled by `checklistService.onDocumentDeleted()`, called as a direct callback from the document service within the same transaction. The reversion clears `completed_by` and `completed_at`, creates an immutable audit record with action `REVERTED_DOC_DELETED`, and triggers notification N3.1 if the task is mandatory. The gatekeeper calculation is immediately affected — if the reverted task was mandatory, submission becomes blocked.
...
- **Events:** N1.1–N1.4 (template workflow), N3.1 (document deleted), N5.1 (template conflict), N8.1 (deferral), N9.1 (ad-hoc internal), N10.1 (ad-hoc client).
...
### Document Service (Deletion Callback)
- **Purpose:** Revert checklist task status when a linked document is deleted.
- **Integration:** After a successful document deletion in `document_service/index.js`, call `checklistService.onDocumentDeleted(trx, workflowId, documentType)`.
- **Approach:** Direct callback (Option A) within the same database transaction for atomicity. The checklist service is imported into the document service as a dependency.
- **Scope:** Only affects instance tasks where `completion_criteria = 'DOCUMENT'`, `document_type` matches, and current `status = 'COMPLETED'`. Tasks in other states (PENDING, DEFERRED) are unaffected.
- **Side effects:** Audit record created (`REVERTED_DOC_DELETED`), notification N3.1 sent for mandatory tasks, gatekeeper immediately recalculated on next check.
...
- The only touch points with existing code are: (a) route registration in `config/routes.js`, (b) `processWorkflow` db_service extensions for the new work type, (c) the gatekeeper hook in CF/Application submission flows, and (d) the document deletion callback in `document_service/index.js`.
...
### Document Service Callback Safety
- The `onDocumentDeleted()` callback is a no-op if no checklist instance exists for the given `workflowId`. It queries for matching tasks first and only acts if results are found.
- If the checklist service is not yet deployed or has no instances, the callback returns immediately with no side effects.
...
| `tradefinance.checklist_instance_task_audit` | Immutable audit trail: task_id, action (COMPLETED, DEFERRED, UNDONE, REVIEWED, REVERTED_DOC_DELETED, etc.), user_id, user_role, timestamp, old_status, new_status, details (JSON — captures declaration text, deferral reason, etc.) |
...
**⛔ STOP — Awaiting Human Review (Draft 4)**

Please:
- Confirm the document-deletion reversion flow has been correctly restored with the direct callback approach
- Verify the `onDocumentDeleted` callback flow and document service integration point are complete
- Confirm `REVERTED_DOC_DELETED` is restored in the audit action list
- Confirm the document service callback safety (no-op when no instances exist) is acceptable
