# ✋ CHECKPOINT 2: Architecture & Design Document (Draft 2)
...
**Revision:** Draft 2 — incorporates feedback from Draft 1
...
|---|---------|--------|--------|
| 1 | Aggregation Engine Design | Replaced single-field (task name) de-duplication with multi-dimensional composite key: `name + stage + category + owner_role + completion_criteria_type` | Feedback: name-only merging incorrectly collapses tasks that serve different purposes at different stages, owners, or criteria types |
| 2 | Aggregation Engine Design | Added document-type sub-key for Document criteria tasks — two tasks requiring different document types are never merged | Feedback: tasks requiring different documents must remain separate even if all other dimensions match |
| 3 | Aggregation Engine Design | Added 4 worked examples illustrating correct vs incorrect merge behavior | Feedback: examples requested for future reference |
| 4 | Configuration Changes | Added `TEXT_RESPONSE` to `COMPLETION_CRITERIA_TYPES` | Feedback: Module 9 (Ask Management) allows text response as a completion criteria |
| 5 | Instance Task State Machine | Added document-deletion reversion flow and clarified the COMPLETED → PENDING transition path | Feedback: clarify task reversion when document is deleted |
...
### Instance Task State Machine (Revised)
...
**Task Reversion on Document Deletion:** When a Document-type task is in `COMPLETED` status and the corresponding document (matched by workflow_id + document_type) is deleted or its status update is received via the checklist API, the task transitions back to `PENDING`. This reversion triggers notification N3.1 (if the task is mandatory) and immediately affects the gatekeeper calculation — if the task was mandatory, the Submit button becomes disabled again. The reversion is recorded in the audit table with action `REVERTED_DOC_DELETED`.
...
## Aggregation Engine Design (Revised)

### Composite De-Duplication Key

The aggregation engine uses a **multi-dimensional composite key** to determine whether two tasks from different templates are true duplicates. Two tasks are considered duplicates **only if ALL** of the following dimensions match:

| Dimension | Column | Comparison |
|-----------|--------|------------|
| Task Name | `name` | Case-insensitive, whitespace-normalized |
...
| Document Type (sub-key) | `document_type` | Exact match — **only compared when completion_criteria = DOCUMENT**. Two tasks requiring different document types are never merged. |

If **any** dimension differs, the tasks are **not duplicates** and both appear independently in the instance.

### Merge Rule for True Duplicates

When two tasks share the same composite key (all dimensions match), they are merged into a single instance task using these rules:

1. **Mandatory wins:** If either task is `is_mandatory = true`, the merged task is mandatory.
2. **Additional Info:** Concatenate from the more-specific template (template with more parameter bindings). If equal specificity, keep the longer `additional_info`.
3. **Sample Document:** Keep sample document from the more-specific template. If equal, keep the first encountered.
4. **Effective Dates:** Use the widest window — earliest `effective_from`, latest `effective_until`.
5. **Source Template:** Record both template IDs on the instance task for traceability.

### Worked Examples

#### Example 1: Same Name, Different Stage — NOT a Duplicate

| Template | Task Name | Stage | Category | Owner | Criteria | Merged? |
|----------|-----------|-------|----------|-------|----------|---------|
...
**Composite Keys:**
- A: `board_resolution|1|DOCUMENTATION|MAKER|DOCUMENT|null`
- B: `board_resolution|2|COMPLIANCE|CHECKER|UNDERTAKING|null`
...
**Composite Keys:**
- A: `verify_insurance|1|DOCUMENTATION|MAKER|DOCUMENT|null`
- B: `verify_insurance|1|DOCUMENTATION|CA|DOCUMENT|null`
...
|----------|-----------|-------|----------|-------|----------|-----------|
| Template A (Global) | "KYC Document" | Maker Draft (priority 1) | Compliance | MAKER | Document (type: KYC) | No |
| Template B (Country: India) | "KYC Document" | Maker Draft (priority 1) | Compliance | MAKER | Document (type: KYC) | Yes |

**Composite Keys:**
- A: `kyc_document|1|COMPLIANCE|MAKER|DOCUMENT|KYC`
- B: `kyc_document|1|COMPLIANCE|MAKER|DOCUMENT|KYC`

**Result:** Keys are identical. Tasks are merged. Mandatory override applies → merged task is **Mandatory = Yes**. Additional info and sample document taken from Template B (more specific — has country parameter).

#### Example 4: Same Name, Different Document Type — NOT a Duplicate
...
**Composite Keys:**
- A: `upload_certificate|1|DOCUMENTATION|MAKER|DOCUMENT|Tax Certificate`
- B: `upload_certificate|1|DOCUMENTATION|MAKER|DOCUMENT|Insurance Certificate`
...
3. MATCH templates against parameterContext:
   a. Templates with NO parameters = Global Fallback → always included
...
6. BUILD composite key for each task:
   → key = normalize(name) + '|' + stage + '|' + category + '|' + owner_role 
           + '|' + completion_criteria
...
8. For each group:
   ...
   b. If group has multiple tasks (true duplicates) → MERGE:
      ...
      - additional_info = from most-specific template (most parameter bindings)
      - sample_document_id = from most-specific template
      - effective_from = MIN(all effective_from in group)
...
Please:
- Confirm the multi-dimensional composite key correctly addresses the duplicate handling concern
- Verify the worked examples match expected behavior
- Confirm `TEXT_RESPONSE` addition to completion criteria types is correct
- Validate the document-deletion reversion flow
