...
**Status:** FEEDBACK DRAFT 1
**Changes:** Developer has clarified and answered ambiguities and assumptions.
**Date:** 2026-03-05  
**PRD:** `prds/current.md`
...
- Clarification: Use the tradefinance schema for checklist related tables.
...
- Clarification: **Important** product_configuration table has nothing to do with workflow types. Please understand this properly. The workflow table in lending schema stores the worflow details and it should be used.
...
- Right, CA is Credit analyst, 'maker' is 'operations maker', HOC is head of credit and so on.
...
- Clarification: The DMS changes for the checklist would be done using checklist apis only. for example: If, during a stage, a document is uploaded and UI captures the checklist task completion automatically, it would send it the the backend through an api with all the details of the checklist instance at that stage (once the user submits).
...
- See what would be the best approach according to you. Ignore all the frontend changes/details/implementations for now.
...
- Dont understand, lets skip this for now.
...
Clarification: the parameter types are dynamically fetched from 'dynamicmodel' schema. These values are seeded using migrations and can be referred from there for model_type 'EXPOSURE'. the parameters that are defined for exposure will be used for our checklist as well.
...
Answer: Yes, these are existing 'WORKFLOW_TYPE' values. Credit facility workflows start with 'CREDIT' and application workflows start with 'FINANCE'.
...
Answer: these values are seeded in the workflow table in 'lending' schema with priorities. For example: for a workflow 'FINANCE_DISBURSEMENT' there might be two records in the table with priority 1 and 2. the user_role with priority 1 will take action first then user_role with priority 2 and so on. So, a stage would be required to represent each record for a workflow.
...
Answer: No, these are to be seeded for stages and will be done at a later stage. See if there is api in the existing code, if not create a todo for this and we will do this at a later stage.
...
Answer: the input fields will be seeded and will be done before the implementation phase. Keep a to do for this for later phases.
...
Answer: the deep-copy is sufficient, but we do need to store history of the interactions with a checklist instance by a user.
...
Answer: Last save wins should be fine.
...
Answer: it is the same backend. The platform is referred to as lending or lms where as client side is referred to as arranging. The database has different schemas for these two. creditfacility is for client side, lendingcreditfacility is for platform. finance is for client side and lendingfinance is for the platform. The checklist service and data will be common for both and will be stored in tradefinance schema. Ask me if you have any doubts in this. this is important
...
Answer: You decide this, I think new would be better. take a call based on your analysis and understanding.
...
Clarification: The document types are currently not stored anywhere in our system. Lets skip this for now.
...
Answer: Yes, a checklist can be only rejected or approved. no option to deactivate is required as of now.
